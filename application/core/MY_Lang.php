...
//class MY_Lang extends CI_Lang {
    ...
    ***************************************************/
    ...
    private $special = array (
        "backend", "shop", "welcome", "signin", "signout"
        //"backend","shop", "welcome", "user", "signin", "signout", "catalog", "order", "setting"
    );
    ...
    function MY_Lang()
    {
        ...
        if ((!empty($this->lang_code)) && (array_key_exists($this->lang_code, $this->languages)))
        {
	     ...
	     if ((!$url_ok) && (!$this->is_special($uri_segment['parts'][0])) && $this->use_redirect ) // special URI -> no redirect
	     {
    ...
    function localized($uri)
    {
     if (!empty($uri))
     {
      ...
      if (!$uri_segment['lang'])
      {
       ...
       if ((!$this->is_special($uri_segment['parts'][0])) && (!preg_match('/(.+)\.[a-zA-Z0-9]{2,4}$/', $uri)) && $this->use_redirect)
       {
                 $uri = $this->lang() . '/' . $uri;
                }
            }
